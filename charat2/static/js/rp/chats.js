    ...
    if (current_mode == mode || first) {
        if ($('body.mobile').length>0) {
            ...
            for(chat in chats) {
                ...
                $('<a>').addClass('title').prop('href','/'+chat.url).text(chat.titlel+(chat.unread ? ' '+chat.url : '')).appendTo('#chat-'+chat.url.replace(/\//g,'-').replace(/\//g,'-'));
                if (chat.type == 'group') {
            ...
            for (var i=0; i<chats.length; i+=2) {
                chat = chats[i];
                $('<div>').addClass('card chat').prop('id', 'chat-'+chat.url.replace(/\//g,'-')).appendTo('#column-1');
                $('<a>').addClass('title').prop('href','/'+chat.url).text(chat.titlel+(chat.unread ? ' '+chat.url : '')).appendTo('#chat-'+chat.url.replace(/\//g,'-'));
                if (chat.type == 'group') {
                    $('<div>').addClass('topic').text(chat.topic).appendTo('#chat-'+chat.url.replace(/\//g,'-'));
                }
            }
            for (var i=1; i<chats.length; i+=2) {
                ...
                $('<a>').addClass('title').prop('href','/'+chat.url).text(chat.titlel+(chat.unread ? ' '+chat.url : '')).append('#column-2');
                if (chat.type == 'group') {
                    $('<div>').addClass('topic').text(chat.topic).appendTo('#chat-'+chat.url.replace(/\//g,'-'));
                }
            }
