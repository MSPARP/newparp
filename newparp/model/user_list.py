...
class ConnectionTokenStore(object):
    ...
    token_key   = "connection_token:%s"

    def __init__(self, redis):
        self.redis = redis

    def create_connection_token(self, chat, user):
        """
        Creates a connection token for the chat and user. Returns a UUID to be
        passed to the front end.
        """
        token = str(uuid.uuid4())
        pipe = self.redis.pipeline()
        pipe.hmset(self.token_key % token, {"chat_id": chat.id, "user_id": user.id})
        pipe.expire(self.token_key % token, self.expire_time)
        pipe.execute()
        return token
    ...
    def use_connection_token(self, token):
        ...
        """
        pipe = self.redis.pipeline()
        pipe.hgetall(self.token_key % token)
        pipe.delete(self.token_key % token)
        data, _ = pipe.execute()
        if not data:
